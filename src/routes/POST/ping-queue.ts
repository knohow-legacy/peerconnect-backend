...
/* Ping the Queue every 30 seconds at __LEAST__ to stay in it!
    ...
    What the API will send back:
    {
        ...
        hasBeenChosenBy: <UserID of Advisor> | null,
        position: Your position in the queue (1-indexed)
    }
*/
...
export async function run(req: express.Request, res: express.Response): Promise<express.Response> {
    ...
    if (!UserQueue.queueHasId(id)) {
        ...
        res.status(200);
        let dupQ = UserQueue.innerQueue[UserQueue.queueIndexOf(id)];
        dupQ[`position`] = UserQueue.queueIndexOf(id) + 1;
        return res.send(dupQ);
    }
}