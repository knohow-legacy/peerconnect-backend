...
export class RouteHandler {
    ...
    async LoadRoutes(): Promise<void> {
        // Router Handler!
        glob(`./${__filename.endsWith(`ts`) ? `src` : `dist` }/routes/**/*.*`, {absolute: false}, (error, files) => {
            ...
            files.forEach(async filePath => {
                ...
                let fileData = require(filePath.replace(`.ts`, ``).replace(`.js`, ``).replace(`${__filename.endsWith(`ts`) ? `src` : `dist` }/`, ``));
